...
import (
	...
	// "github.com/blcvn/lib-golang-test/mpc/tss/multi-party-sig/network"

	"github.com/blcvn/lib-golang-test/mpc/tss/multi-party-sig/test"
...
func CMPSign(c *cmp.Config, m []byte, signers party.IDSlice, n *test.Network, pl *pool.Pool) error {
	...
	fmt.Printf("CPMSign: signature %+v \n", signature)
	return nil
}
...
func All(id party.ID, ids party.IDSlice, threshold int, message []byte, n *test.Network, wg *sync.WaitGroup, pl *pool.Pool) error {
	...
	// CMP KEYGEN
	fmt.Printf("peer %s call CMPKeygen \n", id)
	...
	// CMP REFRESH
	fmt.Printf("peer %s call CMPRefresh \n", id)
	...
	// FROST KEYGEN
	fmt.Printf("peer %s call FrostKeygen \n", id)
	...
	fmt.Printf("peer %s call FrostKeygenTaproot \n", id)
	...
	// CMP SIGN
	fmt.Printf("peer %s call CMPSign \n", id)
	...
	// CMP PRESIGN
	fmt.Printf("peer %s call CMPPreSign \n", id)
	...
	// CMP PRESIGN ONLINE
	fmt.Printf("peer %s call CMPPreSignOnline \n", id)
	...
	// FROST SIGN
	fmt.Printf("peer %s call FrostSign \n", id)
	...
	// FROST SIGN TAPROOT
	fmt.Printf("peer %s call FrostSignTaproot \n", id)
